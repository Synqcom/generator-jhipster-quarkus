        ...
        relationships.forEach(relationship => {
            ...
            if (relationshipType === 'one-to-many' || relationshipType === 'many-to-many') {
                const rewriteFileModelWithRelationships = this.generateFileModel(
                    applicationPropertiesFileName,
                    needle,
                    `quarkus.hibernate-orm.cache."${cacheName}.${relationship.relationshipFieldName}".expiration.max-idle=${CACHE_EXPIRE_AFTER_WRITE}\n` +
                        `quarkus.hibernate-orm.cache."${cacheName}.${relationship.relationshipFieldName}".memory.object-count=${CACHE_MAXIMUM_SIZE}`
                );
